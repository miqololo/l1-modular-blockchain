...
|---|---|---|
| ~~Cross-host (not cross-process) determinism check~~ | Shipped 2026-05-20; codified as `make cross-host-determinism-check`; validated on Apple M3 Pro (Rosetta) ↔ AMD EPYC-Rome | done |
...
- [x] **MVP item #1 — Cross-host determinism shipped.** Codified as `make cross-host-determinism-check` (`REMOTE_HOST=root@<ip>`). The script boots a `linux/amd64` llama-server locally (Rosetta JIT on Apple Silicon; native otherwise), provisions Docker + downloads + SHA-verifies the model on the remote, sends the pinned prompt to each, and asserts all 6 hashes are byte-identical. **First validated 2026-05-20** between Apple M3 Pro (Rosetta) and AMD EPYC-Rome on Ubuntu 26.04 — six identical hashes (`000b7524ab56747f`). The cross-architecture local arm64 hash (`a80f29da55264c0a`) correctly diverges, empirically validating that `hardware_tag` is a load-bearing field. `docs/REPRODUCIBILITY.md` Step 1b documents the protocol; `scripts/cross-host-determinism-check.sh` + `scripts/remote-llama-setup.sh` are the executable artifacts.
